...
from occ_target.mail_send import MailSend
...
class JobConfig(ConfigBase):
    DEFAULT_OCC_TARGET_BATCH_SIZE = 500
    ...
    def get_occ_target_insert_url(self):
        return self.get_child_config().get('env', 'occ_target_insert_url')

    @staticmethod
    def get_occ_target_batch_size():
        return JobConfig.DEFAULT_OCC_TARGET_BATCH_SIZE

    def get_occ_target_mail_receivers(self):
        if EnvUtil.is_prod_env(self.get_env()):
            ...
            return ''.join(
                self.get_child_config().get('email', 'occ_target_mail_receivers').split()).split(';')
