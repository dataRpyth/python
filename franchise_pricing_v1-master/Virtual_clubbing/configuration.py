...
from Virtual_clubbing.mail_send import MailSend
...
class JobConfig(ConfigBase):
    DEFAULT_SHARE_INVENTORY_BATCH_SIZE = 200
    ...
    def get_share_inventory_to_trade_url(self):
        return self.get_child_config().get('env', 'share_inventory_to_trade_url')

    @staticmethod
    def get_share_inventory_batch_size():
        return JobConfig.DEFAULT_SHARE_INVENTORY_BATCH_SIZE

    def get_share_inventory_to_trade_v1_mail_receivers(self):
        if EnvUtil.is_prod_env(self.get_env()):
            ...
            return ''.join(
                self.get_child_config().get('email', 'share_inventory_to_trade_v1_mail_receivers').split()).split(';')

    def get_share_inventory_monitor_receivers(self):
        return ''.join(self.get_child_config().get('email', 'share_inventory_monitor_receivers').split()).split(';')
